...
function itemBaseCny(item) {
  return n(item.quantity) * n(item.unitCny) + n(item.inlandCny);
}
...
function renderTotals() {
  ...
  result.items.forEach((item, index) => {
    ...
    if (el) {
      el.textContent = `중국 원가 ${fmtCny.format(item.cny)} · 원화 ${fmtKrw.format(item.itemChinaKrw)}`;
    }
  });
...
async function importFile(file) {
  ...
  $("#fileName").textContent = `${payload.meta.filename} · ${state.items.length}개 품목 인식`;
  renderAll();
}
